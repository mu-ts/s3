...
export class Metadata {
  ...
  private static _i: Metadata;

  private readonly logger: Logger;
  private readonly registery: { [key: string]: Metadata };
   ...
   */
  public static set<T>(type: string | T, options: { [T in keyof Metadata]: Metadata[T] }): void {
    ...
    Object.keys(options).forEach((key: keyof Metadata) => {
   ...
   */
  public static get<T>(type: T | string): Metadata | undefined {
    const name: string = this.instance.nameFrom(type);
    const metadata: Metadata | undefined = this.instance.registery[name];
  ...
  private static get instance() {
    if (!this._i) this._i = new Metadata();
    return this._i;
  }
   ...
   */
  private defaults(options: { [T in keyof Metadata]: Metadata[T] }): Metadata {
    return {
      ...{
        'id.name': 'id',
